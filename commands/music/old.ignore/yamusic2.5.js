  ...
  async iexec(interact, bot) {
    ...
    if (param === "play") {
      ...
      const search = await player
        .search(query, { searchEngine: ymext, requestedBy: interact.member.id })
        .catch(() => {});
      if (!search?.hasTracks())
        return await interact.editReply("Не найдено треков по этому запросу.");
      if (search.hasPlaylist()) {
        await player
          .play(channel, search.playlist, { searchEngine: ymext })
          .then(() => {
            console.log(
              "[YaMusic] " +
        ...
        await player
          .play(channel, search.tracks[0], { searchEngine: ymext })
          .then(() => {
            console.log(
              "[YaMusic] " +

const discord = require("discord.js"), discordp = require('discord-player')
...
module.exports = {
    ...
    async iexec(interact, bot) {
        ...
        if (param === "skip") {
            ...
            let out = `Список воспроизведения:\n${tracks.map(track=>`${tracks.indexOf(track) + 1}. \`${track.title} - ${track.author}\` requested by @${track.requestedBy.username}`).join("\n")}`
            ...
            if(!eq) return await interact.reply("Не выбран пресет эквалайзера для переключения!")
            await queue.filters.ffmpeg.toggle([eq])
            await interact.editReply(`Фильтр ${eq} переключён на ${queue.filters.ffmpeg.isEnabled(eq)}.`)
        }
    }
}
const discord = require("discord.js"),
  ...
  discordv = require("@discordjs/voice"),
  ym = require("ym-api"),
  fs = require("fs"),
  path = require("path");
...
const play = async (interact, query) => {
  ...
  out = `Добавлено в очередь: \`${track.title} - ${artists.join(", ")} (${
    track.id
  })\``;
  if ((await YaWrapper.getMp3DownloadInfo(parseInt(track.id + ""))).preview)
...
module.exports = {
  ...
  async iexec(interact, bot) {
    ...
    if (trackurl.startsWith("https://")) {
      trackid = parseInt(trackurl.split("/track/")[1] ?? "96614360");
    }
    if (trackid) {

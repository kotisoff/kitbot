import { CommandInteraction, CacheType, ChannelType, VoiceChannel } from "discord.js";
...
export default class VoiceBridgeCommand extends Command {
  ...
  constructor() {
    ...
    this.slashCommandInfo
      ...
      .addStringOption((o) => o.setName("code").setDescription("Code of room."));
  ...
  async runSlash(interaction: CommandInteraction<CacheType>, client: CustomClient): Promise<any> {
    ...
    const Reboot = Command.getCommandByClass<RebootCommand>(client, RebootCommand.prototype);

    // Обработка ввода
    const option = interaction.options.get("option")?.value as "join" | "create" | "leave";
  ...
  // Создание комнаты
  handleCreation(channel: VoiceChannel, code: string | undefined, interaction: CommandInteraction) {
    ...
    const guildRoom = this.rooms.find((room) => room.guilds.includes(channel.guildId));
    if (guildRoom)
      return interaction.reply({
        embeds: [
          CommandEmbed.error(
  ...
  handleConnection(channel: VoiceChannel, code: string | undefined, interaction: CommandInteraction) {
    if (!code)
      return interaction.reply({
    ...
    const totalUsersCount = [...room.channels.values()].map((c) => c.getUsersCount()).reduce((a, b) => a + b, 0);

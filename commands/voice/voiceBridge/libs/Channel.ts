...
import { AudioReceiveStream, VoiceConnection, VoiceConnectionStatus, joinVoiceChannel } from "@discordjs/voice";
...
export default class Channel {
  ...
  constructor(channel: VoiceChannel, room: Room, command: VoiceBridgeCommand) {
    ...
    this.connection.on("stateChange", (_, ns) => {
      if (ns.status == VoiceConnectionStatus.Destroyed || ns.status == VoiceConnectionStatus.Disconnected) {
  ...
  reconfigureInputChannels(channels: Map<string, Channel>) {
    const filteredChannels = [...channels.entries()].filter(([chid, _]) => chid != this.channel.id);

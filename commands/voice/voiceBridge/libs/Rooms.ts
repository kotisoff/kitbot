...
export class Room {
  ...
  addChannel(voiceChannel: VoiceChannel, command: VoiceBridgeCommand) {
    ...
    if (!this.guilds.includes(voiceChannel.guildId)) this.guilds.push(voiceChannel.guildId);
  ...
  reconfigureChannels() {
    this.channels.forEach((channel) => channel.reconfigureInputChannels(this.channels));
  }

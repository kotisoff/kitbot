...
module.exports = {
	...
	async iexec(interact,bot) {
		...
		if(voice.id===channel.id) return await interact.editReply("Вы не можете перенестись в канал в котором вы уже находитесь!")
		channel.members.forEach((u)=>{
			u.voice.setChannel(voice).catch(e=>{return interact.editReply(`Не удалось перенести! ${e.message}`)})
		})
		await interact.editReply(`Перенесено ${channel.members.size} пользователей в <#${voice.id}>`)
	}
}
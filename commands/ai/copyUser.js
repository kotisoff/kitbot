...
command.slashCommandInfo.setDescription("Copies last N user messages to create ai profile based on his messages.")
  .addUserOption(u =>
    u.setName("target")
      .setDescription("Target user to copy.")
      .setRequired(true)
  )
  .addNumberOption(n =>
    n.setName("n")
  ...
  .addStringOption(o =>
    o.setName("prefix")
      .setDescription("Prefix of profile")
  )
  .addStringOption(o =>
    o.setName("system")
      .setDescription("Message from system to ai. System message is more valueable for ai than other.")
  )

command.setSlashAction(async (i, b) => {
  const devGuild = await (await b.guilds.fetch()).get(b.config.bot.devGuildId).fetch();
  const allowedUsers = (await (devGuild).members.fetch());
  if (!allowedUsers.has(i.user.id)) return i.reply(`А ты не ахуел?\nЭта команда доступна только людям с ${devGuild.name}!`);
  ...
  ai.setName(user.displayName).setAvatarUrl(user.avatarURL({ extension: "png" }));

  if (system) ai.addSystemMessage(system);
  if (prefix) ai.setCallPrefix(prefix);

  let messages = (
    await i.channel.messages.fetch({ limit: number }))
    .filter(m => !m.author.bot)
    .reverse()
    .map(m => ({ content: m.content, user: { id: m.author.id, username: m.author.username }, })
    );
  ...
  const files = [new discord.AttachmentBuilder(Buffer.from(JSON.stringify(content), "utf-8"), { name: `${ai.id}.json` })]
  i.reply({ content: `New profile (\`${ai.modid}\`) was saved.\nContent:`, files });

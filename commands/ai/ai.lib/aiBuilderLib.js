const fs = require("fs"),
  path = require("path");
const { get, paths } = require("./aiDataMgr");
const config = get.config;

const modauthor = require("node:os")
  .userInfo()
  .username.split(" ")
  .join("_")
  .toLowerCase();
...
const msgObj = [{ role: roles.user, content: "" }];
...
class Personality {
  ...
  removeMessage = (content = "") => {
    const message = this.presetMessages.indexOf(
      this.presetMessages.filter((i) => i.content == content)[0]
    );
  ...
  build = (write = false, filename = "") => {
    ...
    if (!write) return ai;
    const i = fs
      .readdirSync(paths.modsdir)
      .filter((file) => file.startsWith(this.id)).length;
  ...
  importOld = (ai = oldAi) => {
    ...
    this.setName(ai.name)
      .setAvatarUrl(ai.avatar_url)
      .setCallPrefix(ai.prefix, true)
      .addSystemMessage(ai.personality);
  };
  import = (ai = Ai) => {
    this.author = ai.modid.split(":")[0];
    this.id = ai.modid.split(":")[1];
